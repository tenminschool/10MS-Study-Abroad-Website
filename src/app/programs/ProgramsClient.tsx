...
export default function ProgramsClient({ destinations, universities, programs }: ProgramsClientProps) {
  ...
  const filteredPrograms = enrichedPrograms.filter(prog => {
    ...
    if (activeCategory !== 'all') {
      const hasCategoryMatch = prog.subjects_covered?.some((sub: string) => {
    ...
    if (selectedSubjects.length > 0) {
      const hasSubjectMatch = prog.subjects_covered?.some((sub: string) => selectedSubjects.includes(sub));
